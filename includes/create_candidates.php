...
session_start();
include "db.php";
if(isset($_POST['submit_create_candidate']) && $_SESSION['user_admin'] == "Admin"){
    ...
    if($verifyPassword){
        ...
        if(!empty($_FILES['cand_profile']['name'])){
            ...
            if(in_array($profileType, $allowedType)){
                ...
                $queryCandidate = $conn->prepare("INSERT INTO candidate (profile_pic, candidate_first_name, candidate_last_name, candidate_nickname, Position, Platform) 
                VALUES('$profileName','$cand_fname', '$cand_lname', ' $cand_nickname', '$cand_position', '$cand_platform')");
...
if(isset($_POST['submit_edit_candidate']) && $_SESSION['user_admin'] == "Admin"){
    $confirmPassword = $_POST['passwordConfirm'];
    $userAdmin = $_SESSION['user_admin'];
    $queryPullPassword = $conn->prepare("SELECT Password FROM login WHERE User_Type = '$userAdmin'");
    $queryPullPassword->execute();
